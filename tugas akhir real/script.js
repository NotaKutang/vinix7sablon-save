...
const OrderUtils = {
    // Mendapatkan semua pesanan dari localStorage
    getOrders() {
        try {
    ...
    createOrder(orderData) {
        ...
        const success = this.saveOrders(orders);
    ...
    // Menghasilkan ID unik untuk invoice
    generateId() {
        const now = new Date();
...
function handleOrderSubmit(event) {
    ...
    setTimeout(() => {
        const newOrder = OrderUtils.createOrder(formData);
...
function confirmPayment(orderId) {
    ...
    // Simulasikan delay untuk UX
    setTimeout(() => {
        const success = OrderUtils.updateOrder(orderId, {
            status: 'Selesai',
        ...
        if (success) {
            // Redirect ke halaman sukses dengan parameter orderId
...
function handleTransactionSearch(event) {
    ...
    // Simulasikan delay untuk UX
    setTimeout(() => {
        const order = OrderUtils.getOrderById(invoiceId);
        UIUtils.toggleLoading(false);
...
function displaySuccessInvoice() {
    ...
    container.innerHTML = `
        <div class="invoice-card">
            <div class="invoice-header">
                ...
                <div class="invoice-status status-selesai">${order.status}</div>
            </div>
            ...
            <div class="invoice-details-grid">
                ...
                ${order.buktiPembayaran ? `
                    ...
                    <div class="detail-group">
                        ...
                        <div class="detail-value">${UIUtils.formatDate(order.buktiPembayaran.confirmedAt)}</div>
                    </div>
                ` : ''}
            </div>
        </div>
    ...
    if (printBtn) {
        printBtn.onclick = () => downloadPDFInvoice(orderId);
    }
}
